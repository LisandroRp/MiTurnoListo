// Shared email layout helpers used by the MiTurnoListo transactional templates.
...
export function emailShell({ body, eyebrow, heading, preheader, title }: EmailShellInput) {
  return `
<!DOCTYPE html>
<html lang="es">
  <head>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:600px; background-color:#ffffff; border:1px solid #f1dfd7; border-radius:20px; overflow:hidden; box-shadow:0 12px 36px rgba(23,32,43,0.10);">
            ...
            <tr>
              <td align="center" style="padding:32px 32px 24px 32px; background-color:#faebe5;">
                ...
                <p style="margin:0 0 12px 0; font-size:12px; line-height:18px; letter-spacing:0.08em; text-transform:uppercase; color:#9d381e; font-weight:700;">${escapeHtml(eyebrow)}</p>
                <h1 style="margin:0; font-size:30px; line-height:38px; color:#17202b; font-weight:800;">${escapeHtml(heading)}</h1>
              </td>
            </tr>
            ${body}
            <tr>
...
export function detailCard(rows: DetailRow[]) {
  ...
  return `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#fff8f5; border:1px solid #f1dfd7; border-radius:14px;">
      ${visibleRows.map((row, index) => `
        <tr>
          <td style="padding:${index === 0 ? "20px" : "0"} 20px 8px 20px; font-size:14px; line-height:20px; color:#7b8794; letter-spacing:0.06em; text-transform:uppercase;">${escapeHtml(row.label)}</td>
        </tr>
        <tr>
          <td style="padding:0 20px ${index === visibleRows.length - 1 ? "20px" : "16px"} 20px; font-size:17px; line-height:24px; color:#17202b; font-weight:700;">${escapeHtml(row.value ?? "")}</td>
        </tr>
      `).join("")}
    </table>
  `;
  ...
  return `
    <tr>
      <td align="center" style="padding:8px 32px 32px 32px;">
        <a href="${escapeHtml(href)}" style="display:inline-block; background-color:#ed886e; color:#190e0c; text-decoration:none; font-size:16px; font-weight:800; padding:14px 28px; border-radius:12px;">${escapeHtml(label)}</a>
      </td>
    </tr>
  `;
}
